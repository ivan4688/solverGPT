...
def extract_content(text: Optional[str]) -> str:
    ...
    # Обрабатываем математические конструкции (по имещемуся коду)
    try:
...
def process_math_expressions(text: str) -> str:
    ...
    (Оставлена и расширена логика из предыдущей реализации.)
    """
    if not text:
        return text
...
_RESERVED_TO_ESCAPE_RE = re.compile(r'([\\\[\]\(\)~>#+=\-|{}\.\!])')  # keep '*', '_', '`'
